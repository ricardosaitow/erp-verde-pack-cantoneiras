...
export function ClientesFilter({ onFilterChange, clientesOriginais }: ClientesFilterProps) {
  ...
  const cidades = Array.from(new Set(clientesOriginais.map(c => c.cidade).filter(Boolean))).sort();
  const estados = Array.from(new Set(clientesOriginais.map(c => c.estado).filter(Boolean))).sort();
...
/**
 * Função para aplicar os filtros na lista de clientes
 */
export function applyFilters(clientes: Cliente[], filters: FilterState): Cliente[] {
  return clientes.filter((cliente) => {
    ...
    if (filters.search) {
      ...
      const matchesSearch =
        ...
        cliente.cnpj_cpf?.toLowerCase().includes(searchLower) ||
    ...
    if (filters.tipoPessoa !== 'todos' && cliente.tipo_pessoa !== filters.tipoPessoa) {
      return false;
    }
    ...
    // Filtro de cidade
    if (filters.cidade && cliente.cidade !== filters.cidade) {
      return false;
    }
